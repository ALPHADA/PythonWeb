...
```sh
curl http://127.0.0.1:5000/books/1984
```

책 정보 수정 (PUT):

```sh
curl -X PUT -H "Content-Type: application/json" -d '{"author": "Orwell"}' http://127.0.0.1:5000/books/1984
```

책 삭제 (DELETE):

```sh
curl -X DELETE http://127.0.0.1:5000/books/1984
```
...
201 Created: 요청이 성공적으로 처리되었고, 자원이 생성되었음
400 Bad Request: 클라이언트의 요청이 잘못되었음
401 Unauthorized: 인증이 필요함
403 Forbidden: 요청이 금지되었음
404 Not Found: 요청한 자원을 찾을 수 없음
500 Internal Server Error: 서버에 오류가 발생했음
```
